...
void ZoomFFTExe(uint32_t blockSize)
{
  ...
  if (updateDisplayFlag == true)
  {
    ...
    if (calOnFlag)
    { // Expanded dynamic range during calibration.
      for (int16_t x = 0; x < fftWidth; x++)
      {
        pixelnew[x] = (int16_t)(40.0 * log10f_fast(FFT_spec[x]));
      }
      return; // Array for use in calibration.
    }
    ...
    for (int16_t x = 0; x < fftWidth; x++)
    {
      pixelnew[x] = displayOffset - pixelnew[x] + spectrumMinAvg;
      if (pixelnew[x] > 249)
        pixelnew[x] = 249; // Keep the spectrum inside the box.
      if (pixelnew[x] < 101)
        pixelnew[x] = 101;
    }
  }
...
void CalcZoom1Magn()
{
  if (updateDisplayFlag == true)
  {
    ...
    for (int16_t x = 0; x < SPECTRUM_RES; x++)
    {
      ...
      if (calOnFlag)
      { // Higher dynamic range spectral display during calibration.  The 24 is the legacy "baseOffset".
        pixelnew[x] = static_cast<int16_t>(40.0 * log10f_fast(FFT_spec[x]));
      }
      else
        pixelnew[x] = 24 + (int16_t)(20.0 * log10f_fast(FFT_spec[x]));
    }
    if (calOnFlag)
      return; // No more calculations necssary for calibration.
