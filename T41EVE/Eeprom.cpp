...
void Eeprom::ConfigDataWrite() {
  ...
  Serial.printf("Wrote ConfigData to EEPROM!\n");
}
...
void Eeprom::CalDataWrite() {
  ...
  Serial.printf("Wrote CalData to EEPROM!\n");
}
...
void Eeprom::BandsWrite() {
  ...
  Serial.printf("Wrote Bands to EEPROM!\n");
}
...
Serial.printf("SD card cannot be initialized.\n");
    return 0;
  }
  return 1;
}
...
*****
int Eeprom::SDPresentCheck() {

  // Try to open a file.
  File dataFile = SD.open("SDConfigData.txt");

  if (dataFile) {
    return 1;       // SD card present.
  } else {
    return 0;       // SD card not present.
  }
}
  */