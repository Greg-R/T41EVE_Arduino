...
//      printCalType(autoCal, false);
    ...
    case MenuSelect::UNUSED_1:
      if (IQCalType == 0) {
        ...
        if (calTypeFlag == 1) GetEncoderValueLive(-1.0, 1.0, amplitude, xmitIncrement);
        if (calTypeFlag == 2) GetEncoderValueLive(-1.0, 1.0, iDCoffset, carrIncrement);
      } else {
        IQCalType = 0;
        // Turn off red indication of active setting.
        if (calTypeFlag == 1) GetEncoderValueLive(-1.0, 1.0, phase, xmitIncrement);
        if (calTypeFlag == 2) GetEncoderValueLive(-1.0, 1.0, qDCoffset, carrIncrement);
      }
      break;
...
//            printCalType(autoCal, false);
    ...
    if (IQCalType == 0) amplitude = GetEncoderValueLive(-2.0, 2.0, amplitude, xmitIncrement);
    if (IQCalType == 1) phase = GetEncoderValueLive(-2.0, 2.0, phase, xmitIncrement);
...
ResetFlipFlops();
  ...
  //  }
    ...
    task = MenuSelect::DEFAULT;  // Reset task after it is used.
    //  Read encoder and update values.
    if (IQCalType == 0) iDCoffset = GetEncoderValueLive(-1.0, 1.0, iDCoffset, carrIncrement);
    if (IQCalType == 1) qDCoffset = GetEncoderValueLive(-1.0, 1.0, qDCoffset, carrIncrement);
    if (mode == 0) {
